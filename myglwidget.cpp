    ...
    QGLWidget(QGLFormat(QGL::SampleBuffers), parent)
{
    ...
    xMaterialColor = 0.4, yMaterialColor = 0.1, zMaterialColor = 0.4;
    xLightColor = 0.7, yLightColor = 0.8, zLightColor = 0.1;
    materialShiness = 10;
    xAmbientColor = 0, yAmbientColor = 0.3, zAmbientColor = 0.3;
    lastX = 0, lastY = 0;
...
void MyGLWidget::countFigurePoints() {
    ...
    QVector3D n;
    //qDebug() << curve.size() << endl;
    //qDebug() << curveVertex.size() << endl;
...
void MyGLWidget::paintGL() {
    ...
    for(int i = 0; i < curve.size(); i++) {
        //glVertex2f( curve[i].x() - width() / 2, height()/2 - curve[i].y());
...
//void MyGLWidget::mouseMoveEvent(QMouseEvent *mEvent) {
//    xRot  -= (mEvent->x() - lastX);
//    yRot += (mEvent->y() - lastY);
//    lastX = mEvent->x();
//    lastY = mEvent->y();
